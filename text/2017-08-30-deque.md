...
- > Suppose `O_i` is `pop()` taking the irregular path, and `S` is `steal()` that reads from
...
Crossbeam][current-impl]. More specifically, in the current implementation:

- `push()` issues a release fence and a relaxed write instead of the release write at `'L110`;
- `pop()` loads the buffer earlier;
- `pop()`'s CAS uses SeqCst/Relaxed orderings for success/failure cases instead of Release/Acquire
  orderings;
- `resize()` release-swaps the old buffer with a new one, instead of the release write at `'L307`;
- `steal()` acquire-loads `top` instead of the relaxed load at `'L401`;
...
in the proposed implementation has a stronger ordering for the failure case than that in the current
implementation. But in x86, Power, and ARM architectures, release/acquire orderings for
success/failure cases for a CAS is most likely more efficient than SeqCst/Relaxed orderings.
...
[FIXME(todo)]: https://www.chargueraud.org/research/2013/ppopp/full.pdf
[impl]: https://github.com/jeehoonkang/crossbeam-deque/tree/ordering
