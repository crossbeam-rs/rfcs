...
- If there are multiple `STEAL_EMPTY` invocations, order `I` before `J` if `I -view-> J`. (There can
  be multiple possible linearizations.)
...
If `w = y`, then we have `z = x = w+1` by inequality. Thus `O_i` should have failed the (strong)
compare-and-swap at `'L213`, reading the value written by `S` at `'L410` or a later value. We can
derive a contradiction similarly to the above case.

> Lemma `(VIEW-GROUP)`: FIXME

FIXME.

Now we are ready to prove `(VIEW)`.
...
FIXME: this and the next sections is WIP.
