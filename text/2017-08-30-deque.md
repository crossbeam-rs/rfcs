...
FIXME: introduce the companion PR.
...
- Case 3: `I_i` is `pop()`, and it doesn't take the normal path.
  ...
  b_i`, and it is legit to steal the value from the `top` end of the deque and increment `top`.
...
- Case 5: `I_i` is `steal()` and it returns `EMPTY`.
  ...
  + Case `x = b_i - 1`.
    ...
    release-store at `'L410` to `O_j`'s acquire-load at `'L204`, the value `I` read from `bottom`
