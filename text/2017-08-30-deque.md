...
`bottom`. Let `W_i` be `O_i`'s last write to `bottom`, and `G_i` be the set of those stealers'
invocations that reads `bottom` from the message whose timestamp is within `[Timestamp(W_i),
Timestamp(W_{T+1}))`. (As a corner case, let `G_{-1}` be the set of those invocations that reads
...
For arbitrary `i`, since `O_(i-1)` wrote `W_(i-1)` and `O_i` wrote `W_i`, we have `T(W_(i-1)) <=
view_beginning(O_i)[bottom] < T(W_i)`. Similarly, we have `T(W_i) <= view_end(O_i)[bottom] <
T(W_(i+1))`.

For arbitrary `i` and `S ∈ G_i`, since `G_i` read `W_i` or a later value, we have `T(W_i) <=
view_end(S)[bottom]`. Also, since `G_i` read a value earlier than `W_(i+1)`, we have
`view_beginning(S)[bottom] < T(W_(i+1))`.

> Proof of `(VIEW-OWNER-STEALER)`.

Since `view_beginning(O_i)[bottom] < T(W_i) <= T(W_j) <= view_end(S)[bottom]`, it is not the case
that `S -view-> O_i`.

> Proof of `(VIEW-STEALER-OWNER)`.

Since `view_beginning(S)[bottom] < T(W_(i+1)) <= T(W_j) <= view_end(O_j)[bottom]`, it is not the
case that `O_j -view-> S`.

> Proof of `(VIEW-STEALER-INTER-GROUP)`.

Since `view_beginning(S_i)[bottom] < T(W_(i+1)) <= T(W_j) <= view_end(S_j)[bottom]`, it is not the
...
Suppose that the value of `W_i` is `b`, `S` read the value `x` from `bottom` at `'L404` and `y` from
...
the message with the value `b-1` can be read if `O_i` is a `pop()` and it restored `bottom` at
`'L207` of `'L216`). From the fact that `S` succeeded in stealing a value, we have `y < x <= b` and
thus `y <= b-1`; similarly, from the fact that `S'` failed in stealing a value, we have `w >= z >=
b-1`.
...
- Case 2: `I_i` is `pop()`, and it takes the "normal path" (`len >= 2` and goes to `'L218`).
  ...
  `t_i <= y+1` should obviously hold.) If `I` is `pop()`, then `I` should be linearizaed after `I_i`
  thanks to the coherence of `top`; if `I` is `steal()`, by positive piggybacking synchronization of
  the SC fences of `I_i` and `I`, `I` should load a value that is coherence-after-or `W_i` from
  ...
  b_i`. So it is legit to pop a value from the bottom end of the deque and decrement `bottom`.
...
- Case 3: `I_i` is `pop()`, and it doesn't take the normal path.
  ...
  piggybacking synchronizaztion from `I`'s release-store at `'L410` to `I_i`'s acquire-load at
  `'L204`, the value `I` read from `bottom` at `'L404` should be coherence-before the values written
  by `I_i`. So `I ∈ G_j` for some `j < i`, and thus `I` is linearized before `I_i`.
  ...
  `I` read from `bottom` at `'L404` should be coherence-before the value written by `I_i` at
  `'L216`. (Here, it is important that the CAS at `'L213` is strong.) So `I ∈ G_j` for some `j < i`,
  and thus `I` is linearized before `I_i`. Which means `y+1 <= t_i`, and thus `b_i = x = y+1 <=
  t_i`. Thus it is legit to return `EMPTY`.

  Suppose the CAS at `'L213` succeeds. Let's prove that `t_i <= y`. Consider the invocation `I` that
  writes `y+2` to `top`, if exists. (Otherwise, `t_i <= y` should obviously hold.) If `I` is
  `pop()`, then `I` should be linearizaed after `I_i` thanks to the coherence of `top`; if `I` is
  `steal()`, by positive piggybacking synchronization of the SC fences of `I_i` and `I`, `I` should
  load a value that is coherence-after-or `W_i` from `top`.  Thus `I ∈ G_j` for some `j >= i` and
  `I` is linearized after `I_i`. Thus `t_i <= y` holds. Then `t_i <= y < y+1 = x = b_i`, and it is
  legit to steal the value from the `top` end of the deque and increment `top`.
...
- Case 4: `I_i` is `steal()` and it returns a value.
  ...
  `top` at `'L401`. Let `O` be the last owner method invocation so far whose last write to `bottom`
  is `W_O`, and `v_O = Val(W_O)`. Then `x = v_O` or `x = (v_O)-1` holds by the definition of the
  owner methods `push()` and `pop()`.

  We also prove `t_i <= y` as follows. Consider the invocation `I` that writes `y+2` to `top`. If
  `I` is `pop()` whose last write to `bottom` is `W_I`, then `I` should have read `y+1` from `top`
  at `'L204`. By positive piggybacking synchronization from `I_i`'s release-store at `'L410` to
  `I`'s acquire-load at `'L204`, `x` should be a value coherence-before the `W_I`. It cannot be the
  other write of `y+1` to `bottom` by `I`, as `x >= y+1`.



the value `I` read from `bottom` at `'L404` should be
  coherence-before the value written by `I_i` at

then `I` should be linearized before `I_i` thanks to the coherence of `top`; if
  `I` is `steal()`, by positive piggybacking synchronizaztion from `I`'s release-store at `'L410` to
